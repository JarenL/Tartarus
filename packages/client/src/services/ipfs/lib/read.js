...
const getReadableFileContentStream = ipfsHash => {
  ...
  return new Promise((resolve, reject) => {
    const stream = ipfs.getReadableStream(ipfsHash);

    stream.on('data', file => {
      if (file.type === 'dir')
        return reject(Error(`IPFS hash (${ipfsHash}) is a directory`));
...
const getBase64ImageFromStream = (ipfsHash, progressCallback) => {
  ...
  return new Promise(async resolve => {
    ...
    s.on('end', () => {
      const base64Image =
        'data:image/png;base64,' + arrayBufferToBase64(entireBuffer);
      resolve(base64Image);
    });
  });
};
...
const getBlobFromStream = async (ipfsHash, progressCallback) => {
  ...
  return new Promise(async resolve => {
    ...
    s.on('end', () => {
      ...
      try {
        blob = new this.Blob(arrayBuffer);
      } catch (e) {
