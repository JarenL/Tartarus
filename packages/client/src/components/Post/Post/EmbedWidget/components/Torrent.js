...
class Torrent extends React.Component {
  ...
  addTorrentMedia = async fileIndex => {
    ...
    this.setState({
      showVideo: true,
      activeTorrentFileIndex: fileIndex,
      loadingTorrentFileIndex: null
    });
  ...
  render() {
    ...
    const {
      showVideo,
      status,
      activeTorrentFileIndex,
      loadingTorrentFileIndex,
      filesOpen,
      paused
    } = this.state;
    ...
    for (const file of torrent.files) {
      ...
      let fileProgress = (
        <span className={classes.fileProgress}>
          {status.filesProgress[fileIndex]}
        </span>
      );
      ...
      if (torrent.fileIsStreamable(fileIndex)) {
        ...
        playButton = (
          <IconButton onClick={handlePlay} className={classes.playButton}>
            <PlayArrowIcon />
          </IconButton>
        );
        ...
        mediaFiles.push(
          <p className={fileClassname} key={file}>
            {file} {playButton} {loading} {fileProgress}
          </p>
        );
        continue;
      }
      ...
      const invisiblePlayButton = (
        <IconButton
          className={classnames(
            classes.playButton,
            classes.playButtonInvisible
          )}
        >
          <PlayArrowIcon />
        </IconButton>
    ...
    return (
      <div className={classes.torrent}>
        <Table className={classes.table}>
          <TableBody>
            ...
            <TableRow className={classes.row}>
              ...
              <TableCell>{torrent.sizeInMbs} MB</TableCell>
            </TableRow>

            <TableRow className={classes.row}>
              <TableCell className={classes.nameCell}>Magnet</TableCell>
              <TableCell>
            ...
            <TableRow className={classes.row}>
              <TableCell className={classes.nameCell}>Peers</TableCell>
              <TableCell>{torrent.getStatus().peerCount}</TableCell>
            </TableRow>

            <TableRow className={classes.row}>
              <TableCell className={classes.nameCell}>Files</TableCell>
              <TableCell>
            ...
            {filesOpen && (
              <TableRow className={classes.row}>
                ...
                <TableCell>
                  <div className={classes.filesRow}>{files}</div>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
        ...
        <div
          className={classnames(
            classes.torrentMediaWrapper,
            !showVideo && classes.displayNone
          )}
        >
          <video
            controls
            ref={this.videoRef}
            className={classes.torrentMedia}
          />

          <div className={classes.statusWrapper}>
            <Typography
              style={{ marginLeft: !status.done && 20 }}
              align='left'
              variant='caption'
              gutterBottom
            >
              ...
              <span className={classes.statusItem}>
                Progress: {status.filesProgress[activeTorrentFileIndex]}
              </span>
              {` `}
              <span className={classes.statusItem}>
                Download speed: {status.downloadSpeed}
              </span>
              {` `}
              <span className={classes.statusItem}>
                Upload speed: {status.uploadSpeed}
              </span>
              {` `}
              <span className={classes.statusItem}>
                ETA: {status.filesRemaining[activeTorrentFileIndex]}
              </span>
              {` `}
              {status.filesDone[activeTorrentFileIndex] && (
                <div className={classnames(classes.downloadFile)}>
                  <a
                    onClick={() => torrent.downloadFile(activeTorrentFileIndex)}
                  >
                    ↪Download{' '}
                    <span className={classes.downloadFileName}>
                      {torrent.fileNames[activeTorrentFileIndex]}
                    </span>
                  </a>
                </div>
              )}
            ...
            >
              <Typography
                className={classes.helpTextWrapper}
                align='right'
                variant='caption'
                gutterBottom
              >
                <a href='https://subby.io/encode' target='_blank'>
                  Video not loading?
                </a>
              </Typography>
            </Tooltip>
...
const HelpText = () => (
  <div>
    <p>Files need to download enough pieces to start streaming.</p>
    <p>Only the following codecs can be streamed in the browser:</p>
    <ul>
      <li>vp8</li>
      <li>vorbis</li>
      <li>avc1.4d001e</li>
